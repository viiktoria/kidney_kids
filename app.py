...
if select_1 != scatter_features_1[0] and select_2 != scatter_features_2[0]:
        ...
        st.write(scatter_plot(df[select_1], df[select_2], df['class']))
else:
...
def confusion_plot(cm, ac):
    ...
    ax.xaxis.set_ticklabels(['non-chronic', 'chronic']);
    ax.yaxis.set_ticklabels(['non-chronic', 'chronic']);
    st.pyplot(fig);
    #ac = round(float(ac),5)
    st.write(f'## The Recall is {ac} ##');
    return

st.write(confusion_plot(cm_array, cm['ac']))
